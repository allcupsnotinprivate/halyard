...
Warpweft follows the library convention: it emits through named loggers under
the ``warpweft`` hierarchy and never configures logging itself (a ``NullHandler``
is attached in ``warpweft.core`` so records drop silently until an application
...
to your own handler - warpweft does not install it for you.
"""
